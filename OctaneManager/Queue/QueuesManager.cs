...
namespace MicroFocus.Adm.Octane.CiPlugins.Tfs.Core.Queue
{
    public class QueuesManager
    {
        ...
        /// </summary>
        /// <param name="ev"></param>
        private void EnrichEvents(IList<CiEvent> list)
        {
            foreach (CiEvent ev in list)
            {
                EnrichEvent(ev);
            }
        }
        ...
        private void SendTestResults(CiEvent ciEvent)
        {
            try
            {
                ...
                {
                    var run = _tfsApis.GetRunForBuild(ciEvent.BuildInfo.ProjectId, ciEvent.BuildInfo.BuildId);
                    if (run == null)
                    {
                    ...
                    {
                        var testResults = _tfsApis.GetTestResultsForRun(ciEvent.BuildInfo.ProjectId, run.Id.ToString());
